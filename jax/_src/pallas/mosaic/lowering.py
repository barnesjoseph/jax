...
def _cmp_lowering_rule(prim, ctx: LoweringRuleContext, x, y):
  ...
  # ge(a, b) = a || !b
  #
  # Except for `ne`, all other operations require negation, which is currently
  # not supportd. At present, even if negation were supported, it would still
  # need to be implemented using `select` operations, making it equivalent
  # to our current approach. For more details on negation support,
  ...
  if jnp.issubdtype(dtype, jnp.bool_):
    ...
    if out_aval.shape == ():
      raise NotImplementedError(
          "Scalar boolean comparisons are only supported for `ne`"
      )
